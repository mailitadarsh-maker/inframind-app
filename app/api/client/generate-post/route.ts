  ...
  try {
    ...
    if ((todayCount || 0) >= (client.social_posts_per_day ?? 4)) {
      await supabase.from('generation_logs').insert({
        ...
        type: 'image',
        status: 'skipped',
        engine: client.image_engine || 'flux',
        topic: suggestion || special_day || 'Auto post',
        error_message: `Daily limit reached (${todayCount}/${client.social_posts_per_day ?? 4} posts today)`,
...
Design a professional social media poster. Return ONLY valid JSON:
{
  ...
  "poster_prompt": "Cinematic background image only for a ${isStory ? 'vertical 9:16' : 'square 1:1'} social media poster. Background style using colors ${primary} and ${secondary}. 3D visual elements relevant to ${client.industry}. Cinematic lighting, premium quality. NO TEXT, NO WORDS, NO LETTERS, NO TYPOGRAPHY, NO LOGOS anywhere in the image. Clean empty space on left side for text overlay. Photorealistic, 8K quality."
}`;
    ...
    // Use Llama poster_prompt if available, else build from brand + industry data
    const brandColors = `primary color ${primaryDesc} (${primary}), secondary color ${secondary}`;
    const fullPosterPrompt = concept.poster_prompt
      ? `${concept.poster_prompt}. Brand palette: ${brandColors}. ${client.brand_flux_suffix ? 'Visual style: ' + client.brand_flux_suffix + '.' : ''} CRITICAL: NO TEXT, NO WORDS, NO LETTERS, NO TYPOGRAPHY anywhere in image. No logos, no people, no watermarks. Clean background only.`
      : isLightBrand
        ? `Professional ${client.industry} social media poster background. ${heroVisual}. ${brandColors} color scheme, light elegant background, studio lighting, premium minimal. No text, no logos, no people.`
        : `Professional ${client.industry} social media poster background. ${heroVisual}. ${brandColors} color scheme, dark cinematic background, empty left side for text overlay. 3D render, photorealistic. No text, no logos, no people.`;
    ...
    try {
      ...
      const nvConfigs = [
        { url: 'https://ai.api.nvidia.com/v1/genai/black-forest-labs/flux.1-schnell', body: { prompt: fullPosterPrompt, width: isStory ? 1024 : 1024, height: isStory ? 1344 : 1024, seed: Math.floor(Math.random() * 9999) } },
      ];
      for (const { url, body } of nvConfigs) {
        ...
        try {
          ...
          if (fluxRes.ok) {
            ...
            const b64 = fluxData.artifacts?.[0]?.base64 ?? fluxData.data?.[0]?.b64_json ?? fluxData[0]?.base64 ?? fluxData.image;
      ...
      if (imageBuffer) {
        ...
        const imagePipeline = [
          imageProvider === 'nvidia_flux' ? 'FLUX generate ✓' : 'FLUX generate ✗',
          imageProvider === 'pollinations' ? 'Fallback → Pollinations ✓' : null,
          client.logo_url ? 'Logo composite' : null,
          'SVG text overlay',
          'Supabase upload',
        ].filter(Boolean);
    ...
    // ── Generation log ──────────────────────────────────────────────
    const contentEngine = client.content_engine || 'llama';
    const engineVersionMap: Record<string,string> = {
      llama: 'meta/llama-4-maverick-17b-128e-instruct',
      gpt4: 'gpt-4o',
      claude: 'claude-sonnet-4-6',
    };
    const imageEngineVersion = imageProvider === 'nvidia_flux' ? 'FLUX.1-schnell' : imageProvider === 'pollinations' ? 'Pollinations v2' : 'unknown';
    ...
    await supabase.from('generation_logs').insert({
      ...
      client_name: client.company_name,
      type: 'image',
      status: image_url ? 'success' : (generationError ? 'failed' : 'skipped'),
      engine: imageProvider || 'flux',
      engine_version: imageEngineVersion,
      topic: concept.headline || suggestion || special_day || 'Auto post',
      prompt: fullPosterPrompt?.slice(0, 1000) || null,
      brand_suffix: client.brand_flux_suffix || null,
      post_text: concept.caption || null,
      image_url: image_url || null,
      pipeline: null,
      error_message: generationError || null,
      duration_ms: Date.now() - logStart,
      cost_usd: imageProvider === 'nvidia_flux' ? 0.002 : 0,
      resolution: image_url ? '1024x1024' : null,
    });

    // Log the content generation (Llama/GPT concept)
    await supabase.from('generation_logs').insert({
      client_id,
      client_name: client.company_name,
      type: platform === 'blog' ? 'blog' : 'social',
      status: concept.caption ? 'success' : 'failed',
      engine: contentEngine,
      engine_version: engineVersionMap[contentEngine] || contentEngine,
      topic: concept.headline || suggestion || 'Auto post',
      post_text: concept.caption || null,
      duration_ms: Date.now() - logStart,
      tokens: 800,
      cost_usd: contentEngine === 'gpt4' ? 0.024 : contentEngine === 'claude' ? 0.012 : 0.004,
    });
    // ────────────────────────────────────────────────────────────────
    ...
    try {
      await supabase.from('generation_logs').insert({
        client_id: (await request.json().catch(() => ({})) as any).client_id || null,
        type: 'image',
        status: 'failed',
        engine: 'unknown',
