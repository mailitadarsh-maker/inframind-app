...
export default function LinkedInRewardsPage() {
  ...
  const [pendingAction, setPendingAction] = useState<{ userId: string; action: 'approve' | 'reject' } | null>(null);
  ...
  async function confirmAction() {
    ...
    const res = await fetch('/api/linkedin/admin-action', {
      ...
      body: JSON.stringify({ userId, action, adminNote: adminNoteInput }),
    });
    ...
    if (res.ok) {
      ...
      showToast(
        action === 'approve' ? '14 days added to user account.' : 'Submission rejected.',
        'success'
      );
    } else {
  ...
  if (checkingAdmin) {
    return (
      <div style={{
        minHeight: '100vh', background: '#09090f', color: '#fff',
  ...
  if (!isAdmin) {
    return (
      <div style={{
        minHeight: '100vh', background: '#09090f', color: '#ef4444',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
    ...
    <div style={{
      ...
      {pendingAction && (
        ...
        >
          ...
          >
            <div style={{ fontSize: '16px', fontWeight: 700, color: '#f9fafb', marginBottom: '8px' }}>
              {pendingAction.action === 'approve' ? '✅ Approve Submission' : '❌ Reject Submission'}
            </div>
            ...
            <textarea
              ...
              placeholder={pendingAction.action === 'approve' ? 'Optional note…' : 'Rejection reason…'}
              rows={3}
              autoFocus
              style={{
      ...
      <div style={{ marginBottom: '36px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <h1 style={{ fontSize: '28px', fontWeight: 700, letterSpacing: '-0.03em', color: '#f9fafb', margin: 0 }}>
            LinkedIn Reward Requests
          </h1>
          <div style={{
            display: 'flex', alignItems: 'center', gap: '8px',
            background: '#0d1117', border: '1px solid #1f2937',
            borderRadius: '8px', padding: '6px 14px', fontSize: '13px', color: '#6b7280',
          }}>
            <span style={{
      ...
      {loading ? (
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          padding: '80px 0', color: '#374151', fontSize: '14px', gap: '10px',
        }}>
          <span style={{
            width: '16px', height: '16px', border: '2px solid #1f2937',
            borderTopColor: '#22c55e', borderRadius: '50%', display: 'inline-block',
        ...
        <div style={{ background: '#0d1117', border: '1px solid #1f2937', borderRadius: '16px', overflow: 'hidden' }}>

          {/* Table Header */}
          <div style={{
            display: 'grid', gridTemplateColumns: '1fr 1.6fr 1.2fr 160px',
          ...
          {submissions.map((row, i) => (
            ...
            >
              <div>
                <code
                  style={{
                    fontSize: '12px', color: '#9ca3af', background: '#111827',
                    padding: '3px 8px', borderRadius: '6px', border: '1px solid #1f2937',
              ...
              <div>
                <a
                  ...
                  style={{
                    color: '#22c55e', fontSize: '13px', textDecoration: 'none',
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  style={{
                    ...
                    background: actionLoading === row.id + 'approve'
                      ? 'rgba(34,197,94,0.2)' : 'rgba(34,197,94,0.15)',
                ...
                <button
                  ...
                  style={{
                    padding: '6px 14px', borderRadius: '8px',
                    background: actionLoading === row.id + 'reject'
